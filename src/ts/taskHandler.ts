...
function outputHandler(text: string, loglevel: string = "info") {
	let output = $(".output");
...
function startTask(result: any, callback: any) {
  let output = $(".output");
...
ipc.on("start-handler", function (event: Event, fromWindowId: number, project: ProjectInterface, tasksWithOptions: Array<any>) {
  ...
  async.seq(...jobs)(true, function (err: string, foo: string) {
...
function onCheckTasks(event: Event, parentWindowId: number, tasks: Array<any>, currentApplicationDirectory: string, debug: boolean = false) {
  $(".js-close").on("click", window.close);
...
function onVerifyAll(event: Event, parentWindowId: number, userDir: string, projectsPath: string, debug: boolean = false) {
  let output = $(".output");
...
function onDeployApp(parentWindowId: number, absolutePath: string, applicationId: string, version: string, debug: boolean = false) {
...
function onDeployIndexes(parentWindowId: number, absolutePath: string, applicationId: string, debug: boolean = false) {
...
function onMigrateVersion(parentWindowId: number, absolutePath: string, applicationId: string, version: string, debug: boolean = false) {
  let output = $(".output");
...
ipc.on("start-deploy", function (event: Event, parentWindowId: number, absolutePath: string, applicationId: string, version: string) {
  $(".js-close").on("click", window.close);
...
ipc.on("start-update-indexes", function (event: Event, parentWindowId: number, absolutePath: string, applicationId: string) {
  $(".js-close").on("click", window.close);
...
ipc.on("start-migrate-version", function (event: Event, parentWindowId: number, absolutePath: string, applicationId: string, version: string) {
  $(".js-close").on("click", window.close);
...
function addProject(newProjectName: null|string, parentWindowId: number) : any {
  let output = $(".output");
  ...
  proc.on('close', (code: number) => {
    ...
    if (code === 0) {
      ...
      proc2.stdout.on("data", (chunk: WriteStream) => {
        let data = chunk.toString();
        if (data) {
          outputHandler(data);
        }
      ...
      proc2.stderr.on("data", (chunk: WriteStream) => {
        let data = chunk.toString();
        if (data) {
          outputHandler(data);
        }
      ...
      proc2.on('close', function (code: number) {
...
function onCheckGcloudUpdate(event: Event, parentWindowId: number, debug: boolean = false) {
  // TODO: need a check update command
  let cmdTemplate = `gcloud --format json components update`;
}

function onUpdateGcloud(event: Event, parentWindowId: number) {
  $(".js-close").on("click", window.close);
  let foregroundColor = settingsStorage.get("terminal_foreground_color", "#00ff00");
  let backgroundColor = settingsStorage.get("terminal_background_color", "#000000");
  let output = $(".output");
...
function onCheckAppengineStatus(event: Event, parentWindowId: number, applicationId: string, debug: boolean = false) {
  $(".js-close").on("click", window.close);
  let foregroundColor = settingsStorage.get("terminal_foreground_color", "#00ff00");
  let backgroundColor = settingsStorage.get("terminal_background_color", "#000000");
  let output = $(".output");
  ...
  exec(cmdTemplate, function (error: string, stdout: string, stderr: string) {
    const fromWindow = BrowserWindow.fromId(parentWindowId);
    if (error) {
...
function onGcloudAuthStatus(event: Event, parentWindowId: number, debug: boolean = false) {
  $(".js-close").on("click", window.close);
  let foregroundColor = settingsStorage.get("terminal_foreground_color", "#00ff00");
  let backgroundColor = settingsStorage.get("terminal_background_color", "#000000");
  let output = $(".output");
...
function onGetAppengineRegions(event: Event, parentWindowId: number) {
  $(".js-close").on("click", window.close);
  let foregroundColor = settingsStorage.get("terminal_foreground_color", "#00ff00");
  let backgroundColor = settingsStorage.get("terminal_background_color", "#000000");
  let output = $(".output");
  ...
  exec(cmdTemplate, function (error: string, stdout: string, stderr: string) {
    ...
    try {
      let rawData1 = JSON.parse(stdout.toString());
      let sortedRegions = rawData1.sort((a: any, b: any) => {
...
function onCreateAppengine(event: Event, parentWindowId: number, applicationId: string, region: string, debug: boolean = false) {
...
ipc.on("add-project", function (event: Event, newProjectName: string, parentWindowId: number) {

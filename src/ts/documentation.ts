...
function topicSelected(event: Event) {
...
function activateData(view: string) {
  if (!!view) {
...
ipcRenderer.on("start", function(event: Event, view: string, userDir: string) {
...
ipcRenderer.on("change", function (event: Event, view: string) {

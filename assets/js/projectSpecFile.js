...
function createGulpTask(projectPath, taskPath) {
    ...
    return new ProjectSpecTask(`gulp in ${dirname}`, `gulp-in-${dirname.replace(" ", "_").replace("/", "_")}`, "gulp", dirname, taskArguments);
}
function createViTask(projectPath) {
    ...
    return new ProjectSpecTask("Build Vi", "build-vi", "make", "vi", taskArguments, [
        ["exists", "${currentApplicationDirectory}/vi/_pyjs.js"]
    ]);
}
function createMakeTask(projectPath, taskPath) {
    return new ProjectSpecTask("Build Vi", "build-vi", "make", "vi", []);
}

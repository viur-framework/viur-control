...
function getProjectVersions(event, refresh = false) {
...
function removeApplicationIdFromProject(event) {
...
function checkGcloudAuthStatusResponse(event, status, accounts, errors) {
...
function searchProject(event) {
    ...
    if (keyCode === 38) {
        if (!currentInternalId) {
            ...
            $(container).scrollTop($(thisElement).offset().top - $(container).offset().top + $(container).scrollTop());
...
function fillNextVersion(initials) {
    if (!initials) {
...
function initLabelIconCache(event) {
    ...
    let storedLabels = labelIconsStorage.get("allLabels", []);
    for (let entry of storedLabels) {
        let clone = { title: entry.title, path: entry.path, id: 0 };
        if (clone.path) {
    ...
    labelIconList.sort(labelSettingsLib_1.LabelInternalInterfaceSorter);
...
function requestDiscoverLabelIcons(event) {
...
function hideVcLogger(event) {
    loggerWindow.hide();
}
function startVcLogger(event) {
...
function onRequestGcloudProjectsResponse(event, data, update) {

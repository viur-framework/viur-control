...
class Project {
    constructor(directoryName, absolutePath, serverPort, adminPort, internalId, tasks = undefined, applicationIds = undefined, appengineDirectories = undefined, credentials = undefined, projectIcon = undefined, custom_devserver_cmd = undefined) {
...
function scanProject(directoryName, windowId, callback, oldProjectsByName = undefined, isNew = false, subprocessIds = undefined) {
    ...
    fastGlob('**/app.yaml', {
    ...
    }).then(function (appengineDirectories) {
        ...
        try {
            if (appengineDirectories.length > 0) {
                ...
                const myProject = new Project(directoryName, absolutePath, serverPort, adminPort, internalId, tasks, applicationIds, result, credentials, projectIcon, custom_devserver_cmd);
...
function scanNewProject(event, directoryName, windowId) {
    console.log("scanNewProject", directoryName, windowId);
    $("title").text(`ViUR control - scan new project ${directoryName}`);
    scanProject(directoryName, windowId, function (newProject) {
        if (!newProject) {
            return;
        }
...
function getProjects(event, windowId, subprocessIds = undefined, debug = false) {
    if (debug) {
...
function loadStoredProjects(event, windowId, debug = false) {
    $(".js-close").on("click", window.close);
    ...
    for (let project of storedProjects) {
        ...
        projects.push(new Project(project.directoryName, project.absolutePath, project.serverPort, project.adminPort, project.internalId, project.tasks, project.applicationIds, project.appengineDirectories, project.credentials, project.projectIcon, project.custom_devserver_cmd));
    }
...
function applicationIdSorter(a, b) {
    if (a.name < b.name)
        return -1;
    if (a.name > b.name)
        return 1;
...
function onRequestVersions(event, windowId, applicationId, debug = false) {
    if (debug) {
        $(".js-close").on("click", window.close);
    }
    ...
    let proc = exec(cmdTemplate, { "shell": true }, function (error, stdout, stderr) {
...
function onIndexesCheck(event, parentWindowId, project, debug = false) {
    ...
    exec(cmdTemplate, { "cwd": projectPath }, function (error, stdout, stderr) {

...
function onCheckTasks(event, parentWindowId, tasks, currentApplicationDirectory, debug) {
  $(".js-close").on("click", window.close);
...
function onVerifyAll(event, parentWindowId, userDir, projectsPath, debug) {
  let output = $(".output");
...
function onGetDomainMappings(event, parentWindowId, applicationIds, debug) {
  $(".js-close").on("click", window.close);
  let foregroundColor = settingsStorage.get("terminal_foreground_color", "#00ff00");
  let backgroundColor = settingsStorage.get("terminal_background_color", "#000000");
  let output = $(".output");
